...
# rostopic pub /ardrone/pos_ref geometry_msgs/Vector3 '{x: 0.0, y: 0.0, z: 1.0}'
...
class AttitudeControl:
    ...
    def run(self):
        ...
        while not rospy.is_shutdown():
            ...
            roll_output = self.pid_roll_rate.compute(roll_rate_sp, self.euler_rate_mv.x)

            # Pitch control
            # Outer loop - pitch 
            pitch_rate_sp = self.pid_pitch.compute(self.euler_sp.y, self.euler_mv.y)
            # Inner loop - pitch rate 
            pitch_output = self.pid_pitch_rate.compute(pitch_rate_sp, self.euler_rate_mv.y)

            # Normalize yaw ref to [-π, π]
            ...
            mot_sp4 = w_hover - roll_output + pitch_output + yaw_output
