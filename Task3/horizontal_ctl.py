...
# rosrun urs_solution test_horizontal.py
...
class HorizontalControl:
    ...
    def run(self):
        ...
        while not rospy.is_shutdown():
            ...
            pitch_sp_global = self.pid_vx.compute(vx_sp, self.vx_mv) 

            vy_sp = self.pid_y.compute(self.y_sp, self.y_mv)
            roll_sp_global = -self.pid_vy.compute(vy_sp, self.vy_mv)  

            # for global frames - pitch and roll decoupling w.r.t measured yaw
            pitch_sp = math.cos(self.yaw_mv) * pitch_sp_global - math.sin(self.yaw_mv) * roll_sp_global
            roll_sp = math.sin(self.yaw_mv) * pitch_sp_global + math.cos(self.yaw_mv) * roll_sp_global
